...
class TrainManager:
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) \
            ...
            for batch in iter(train_iter):
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    valid_score, valid_loss, valid_references, valid_hypotheses, \
                        valid_inputs, all_dtw_scores, valid_file_paths, valid_pck_score = \
                        validate_on_data(
                    ...
                    if self.is_best(ckpt_score):
                        ...
                        # display = list(range(0, len(valid_hypotheses), int(np.ceil(len(valid_hypotheses) / 6))))
                    ...
                    self._add_report(valid_pck=valid_pck_score,
                    ...
                    self.logger.info(
                        'Validation result at epoch %3d, step %8d: Val DTW Score: %6.2f, Val PCK Score: %6.2f, '
                        'loss: %8.4f,  duration: %.4fs',
                            epoch_no+1, self.steps, valid_score, valid_pck_score,
        ...
        self.logger.info('Best validation result at step %8d: %6.2f %s PCK%6.2f.',
                         self.best_ckpt_iteration, self.best_ckpt_score,
                         self.early_stopping_metric, best_pck)
    ...
    def _add_report(self, valid_pck: float, valid_score: float, valid_loss: float, eval_metric: str,
        ...
        if report_type == "val":
            with open(self.valid_report_file, 'a') as opened_file:
                opened_file.write(
                    "Steps: {} Loss: {:.5f}| DTW: {:.3f}| PCK: {:.3f}"
                    " LR: {:.6f} {}\n".format(
                        self.steps, valid_loss, valid_score, valid_pck,
...
def test(cfg_file,
         ckpt: str, save_output=False) -> None:
    ...
    data_to_predict = {"test": test_data, "dev": dev_data}
    ...
    for data_set_name, data_set in data_to_predict.items():
        ...
        inputs, all_dtw_scores, file_paths, pck_score = \
            validate_on_data(
                model=model,
                data=data_set,
        ...
        f.write(f'{data_set_name}\n')
        f.write("Loss: " + str(loss) + " DTW: " + str(score) + " PCK: "+ str(pck_score))
        f.close()
        print(data_set_name, ": ", loss, score, pck_score)

        # Set which sequences to produce video for
        # display = list(range(len(hypotheses)))
        display = list(range(20))
        ...
        if save_output:
            if not os.path.exists(model_dir + "/test_hypotheses/"):
                os.mkdir(model_dir + "/test_hypotheses/") 

            hyps = []
            for i in range(len(hypotheses)):
                hyps.append(hypotheses[i][1:, :])
            tensor = torch.cat(hyps, dim=0)
            print('tensor shape', tensor.shape)
            torch.save(tensor, f'{model_dir}/test_hypotheses/tensor.pt')

            for i in range(len(hypotheses)):
                np.savetxt(f'{model_dir}/test_hypotheses/{i}.txt', hypotheses[i].cpu().numpy())
                # print('hyp', hypotheses[i].cpu().numpy().shape)
                # print('ref', references[i].cpu().numpy().shape)
                # print(file_paths[i])
                # print('--')
                # np.savetxt(f'{model_dir}/test_hypotheses/{i}_path.txt', file_paths)
            print(f'Saved {i+1} hypotheses')
