...
def calculate_pck(pred, gt, alpha=0.2):
    """
    Transform the keypoints from (N, M, K) to (N, 2, K/3) by splitting X and Y coordinates and calculates PCK.
    
    :param pred: predicted keypoints of shape (N, M, K).
    :param gt: ground truth keypoints of shape (N, M, K).
    ::return: PCK radius for each keypoint in each sample.
    """

    # Assuming pred and gt have shapes (N, 3K) where K is the number of keypoints
    N = pred.shape[0]
    K = pred.shape[1] // 3  # Number of keypoints

    # Reshape and split the keypoints
    pred = pred.reshape(N, K, 3).transpose(0, 2, 1)
    gt = gt.reshape(N, K, 3).transpose(0, 2, 1)
...
def compute_pck_radius(gt, alpha=0.2):
    """
    Compute the PCK radius for each sample.

    :param gt: Ground truth keypoints of shape (N, 2, K).
    :param alpha: Scaling parameter.
    :return: PCK radius for each keypoint in each sample.
    """
    # Compute the width and height for each sample
    width = np.max(gt[:, 0, :], axis=1) - np.min(gt[:, 0, :], axis=1)
    height = np.max(gt[:, 1, :], axis=1) - np.min(gt[:, 1, :], axis=1)
    
    # Determine the maximum axis length
    max_axis = np.maximum(width, height)
    
    # Compute the PCK radius for each keypoint
    pck_radius = max_axis[:, np.newaxis] * alpha
